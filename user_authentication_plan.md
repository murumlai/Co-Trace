...
Replace the current admin/password login with GitHub OAuth, but do it together with per-user authorization. The original OAuth-only plan would authenticate users but would not isolate job data, cache deletion permissions, or write permissions. Because database access is only possible from inside the corporate network, GitHub OAuth is required for identity but GitHub org membership does not need to be verified. The recommended rollout is: mandatory GitHub OAuth sign-in, httpOnly 30-day session cookies for persistence, job ownership checks for each user, shared read-through analysis cache for same-product matches, protected admin cache entries, optional fresh LLM reruns, and shared Product Knowledge with admin-only writes.

**Steps**

1. Register and configure the GitHub OAuth app.
   - Set local callback URL to `http://localhost:8000/api/auth/github/callback`.
   - Request only the scopes needed to identify the signed-in GitHub user. Do not request `read:org` unless admin team lookup is later required.
   - Add env vars: `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`, `JWT_SECRET`, `FRONTEND_URL`, `COOKIE_SECURE`, and an admin allowlist such as `GITHUB_ADMIN_USERS`.
...
3. Replace password auth with GitHub OAuth in the backend.
   - In `backend/app/auth.py`, replace `SimpleAuth` with GitHub OAuth helpers for authorize URL generation, code exchange, GitHub user lookup, JWT creation, and JWT verification.
   ...
   - Include at least GitHub login, GitHub id, and admin flag in the session claims or in a derived auth context.

4. Add OAuth routes.
   - In `backend/app/main.py`, remove `POST /api/login`.
   - Add `GET /api/auth/github` to create a CSRF state cookie and redirect to GitHub.
   - Add `GET /api/auth/github/callback` to validate state, exchange code, fetch the GitHub user identity, set the 30-day `session` cookie, and redirect to the frontend.
...
6. Implement shared read-through analysis cache with ownership-protected deletion. Depends on step 3.
   - In `backend/app/analysis_cache.py`, keep cache lookup shared across authenticated corporate-network users for matching product/code/context/signature/knowledge-version entries so User B can reuse a previous User A or admin result without a fresh LLM call.
...
7. Keep Product Knowledge shared, but protect writes with admin authorization.
   - Product Knowledge and acronym data remain shared reads for all authenticated users.
   - Write routes under `/api/knowledge/*` should require admin authorization.
   - Admin status should come from an explicit allowlist such as `GITHUB_ADMIN_USERS`; GitHub team membership can be added later only if the needed scope is acceptable.
   - Read routes can remain available to all authenticated users.
...
9. Update tests for auth, ownership, and roles.
   ...
   - Add tests that non-admin authenticated users can read Product Knowledge but cannot upload, update, rebuild, or delete it.
   - Add tests for expired/invalid session cookies, OAuth callback state mismatch, GitHub identity lookup failure, and logout cookie clearing.
...
3. Complete OAuth with a GitHub account and confirm `/api/me` returns the expected login and role.
...
7. Confirm a normal authenticated user cannot delete admin-saved/protected cache entries, but can run a fresh LLM analysis when choosing the cache bypass option.
8. Confirm a normal authenticated user can read Product Knowledge but receives `403` from Product Knowledge write/rebuild/delete routes.
9. Confirm a configured admin user can perform Product Knowledge writes.
10. Confirm any GitHub-authenticated user on the corporate network can sign in, while failed OAuth or identity lookup attempts redirect back with a clear login error.

**Decisions**

- GitHub OAuth replaces the current admin/password login entirely.
- GitHub OAuth is mandatory for all users, but the backend does not need to verify GitHub org membership because database access is already limited to the corporate network.
...
- Product Knowledge and acronyms are shared across authenticated users for reading.
- Product Knowledge writes are admin-only.
- Admin authorization should use a configured allowlist such as `GITHUB_ADMIN_USERS`. GitHub team membership is optional later and would require adding the appropriate GitHub scope.

**Further Considerations**

1. Admin source: simplest is `GITHUB_ADMIN_USERS`; GitHub team membership such as `GITHUB_ADMIN_TEAM` is optional later if the app can request the needed scope.
