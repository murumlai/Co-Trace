...
class LexicalKnowledgeRetriever:
    ...
    def retrieve(self, record: UnitRecord) -> KnowledgeContext:
        ...
        entries = index.by_product.get(product_code)
        if not entries:
    ...
    def _score(
        ...
        for entry in entries:
            ...
            if overlap > 0:
                score = overlap
                if entry.category == "debug_learning":
...
def _assemble_context(matches: list[RetrievalMatch]) -> tuple[str, str]:
    ...
    Returns ``(all_context, debug_learning_only)``. Debug-learning known-failure
    text is surfaced separately so prompts can present it as higher-priority
    historical evidence.
    """
    ...
    used = 0
    # Debug-learning first so it survives truncation.
    ordered = sorted(matches, key=lambda m: 0 if m.category == "debug_learning" else 1)
    for match in ordered:
        ...
        blocks.append(block)
        used += len(block)
        if match.category == "debug_learning":
